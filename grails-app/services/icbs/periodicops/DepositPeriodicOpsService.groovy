...
class DepositPeriodicOpsService {
    ...
    def monthlyFDInterest(Date currentDate, Branch branch, UserMaster user) {
        ...
        for(deposit in depositList) {
            ...
            deposit.accruedIntPayable = fdRoll.normalInterestAmt
            deposit.accruedTaxPayable = fdRoll.taxAmt1  
            deposit.save()   
            ...
            if((fd.currentRollover.endDate <= currentDate) && (fd.currentRollover.status == RolloverStatus.read(1))){ 
                ...
                if(fd.currentRollover.type.id==2){
                    ...
                    if(fd.currentRollover.interestPaymentMode.id==2){
                        ...
                        def nextDate = depositService.getNextDate(currentDate,currentDate,fd.maturityDate)
            ...
            for (d in dl) {
                ...
                if (d.lmAveBalAmt < d.product.depositChargeStart || d.status.id == 5) {
                    ...
                    if (d.status.id != 5) {
                        ...
                        dOld = MonthlyBalance.findByRefDateAndAccountNo(lastDateOfPreviousMonth,d.acctNo)
                        if (dOld) {
                            if (dOld.lmAveBalAmt > d.product.depositChargeStart) {
                                chgAmt = 0
                            }
                        } else {
                            chgAmt = 0
    ...
    private def depositInterestCalculation(Date currentDate,Branch branch){
        ...
        for(deposit in depositList){ 
            ...
            if(deposit.depositInterestScheme.depositInterestCalculation.id ==1 ){
                ...
                deposit.accruedIntForTheMonth = interestBalAmt
                deposit.accruedIntPayable += interestBalAmt
                deposit.accruedTaxForTheMonth =  interestBalAmt * deposit.depositTaxChargeScheme.taxRate.div(100)
                deposit.accruedTaxForTheMonth = deposit.accruedTaxForTheMonth.round(2)
                deposit.accruedTaxPayable += (interestBalAmt * deposit.depositTaxChargeScheme.taxRate.div(100)) 
