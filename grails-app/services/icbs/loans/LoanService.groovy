...
def generateAccountNo(Loan loanInstance) {
        ...
        */
        ...
        def result = Loan.executeQuery("select max(accountNo)from Loan where branch = ? and product = ?)", [loanInstance.branch, loanInstance.product])
        int serialNum
        println '+++' + result
        String serialOld 
        if (result[0] != null) {
    ...
    def beforeValidation(Loan loanInstance) { 
        if (loanInstance.loanApplication) {       
            loanInstance.branch = loanInstance?.customer?.branch
    ...
    def saveLoan(Loan loanInstance, def installmentAmount) {
        ...
        println '+++' + loanInstance.accountNo
        loanInstance.save flush:true
        if(Institution.get(65).paramValue =='TRUE') {
    ...
    def saveLoanAmendment(Loan loanInstance, UserMaster user) {
       ...
       if (loanInstance.getPersistentValue('loanPerformanceId') != loanInstance.loanPerformanceId) {
    ...
    def updateBranch(Loan loanInstance, Branch branch) {
    ...
    def generateInstallmentSchedule(Loan loanInstance, Double installmentAmountOverride) {
        ...
        def totalDeductionsEir = 0
        for(loanDeduction in loanInstance?.loanDeductions) {
            if (loanDeduction?.scheme?.hasEirMode == true) {
                totalDeductionsEir += loanDeduction?.amount    
            }
        }
        def adv = 0
        ...
        if (installmentCalculation == 3 && loanInstance.frequency.id == 3) {
        ...
        loanInstance.totalNetProceeds = amount - totalDeductions  
