...
<div>
    ...
    <div name="add-sweep-account-form">
        ...
        <div class="fieldcontain form-group ${hasErrors(bean: sweepAccount, field: 'type', 'has-error')}">
            <label for="type" class="control-label col-sm-4">Type</label>
            <div class="col-sm-8">
                <g:select class="form-control" id="type" name="type.id" from="${icbs.lov.SweepType.list().sort{it.id}}" optionKey="id" optionValue="description" value="${sweepAccount?.type?.id}" onchange="updateSweepForm()"/>
            </div>
        </div>

        <div class="fieldcontain form-group ${hasErrors(bean: sweepAccount, field: 'rule', 'has-error')}">
            <label for="rule" class="control-label col-sm-4">Rule</label>
            <div class="col-sm-8">
                <g:select class="form-control" id="rule" name="rule.id" from="${icbs.lov.SweepRule.list().sort{it.id}}" optionKey="id" optionValue="description" value="${sweepAccount?.rule?.id}" onchange="updateSweepForm()"/>
            </div>
        </div>

        <div id="fund-limit-amount-form-group" class="fieldcontain form-group ${hasErrors(bean: sweepAccount, field: 'fundLimitAmt', 'has-error')}">
            <label class="control-label col-sm-4" for="fundLimitAmt">Fund Limit Amount <span class="required-indicator">*</span></label>
            <div class="col-sm-8">
              
                <g:field type="number" name="fundLimitAmt" value="${fieldValue(bean: sweepAccount, field: 'fundLimitAmt')}" class="form-control truncated"/> 
                <g:hasErrors bean="${sweepAccount}" field="fundLimitAmt">
                    <div class="controls">
                        <span class="help-block">
                            <g:eachError bean="${sweepAccount}" field="fundLimitAmt">
                                <g:message error="${it}" /><br/>
                            </g:eachError>
                        </span>
                    </div>
        ...
        <div id="fund-limit-percentage-form-group" class="fieldcontain form-group ${hasErrors(bean: sweepAccount, field: 'fundLimitPercentage', 'has-error')} required">
