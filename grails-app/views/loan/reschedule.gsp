...
<html>
	<head>
		...
		<g:set var="entityName" value="${message(code: 'loan.label', default: 'Loan')}" />
		    <g:if test="${module == "loanRestructure"}">
        	<title>Restructure Loan Account</title>
                    </g:if>
        <g:else>
        	<title>Reschedule Loan Account</title>
        </g:else>  		
		<g:javascript>
			...
			function updateLoanApplicationAjax(params) {
				...
				$.ajax({
				    ...
				    success: function(msg){						
						...
						$('#grantedAmount').val($(msg).find('#loan-amount').html());
            ...
            function addInstallmentAjax() {
				$.ajax({
				    type: 'POST',
				    data: {date:$('#installmentDate').val(), principal:$('#principalInstallmentAmount').val(), interest:$('#interestInstallmentAmount').val()},
            ...
            function updateInstallmentAjax() {
				$.ajax({
				    type: 'POST',
				    data: {id:$('#installmentId').val(), date:$('#installmentDate').val(), principal:$('#principalInstallmentAmount').val(), interest:$('#interestInstallmentAmount').val()},
            ...
            function addSweepAccountAjax() {        		
				$.ajax({
				    type: 'POST',
				    data: {deposit:$('#depositAccount').val(), type:$('#type').val(), rule:$('#rule').val(), fundLimitAmt:$('#fundLimitAmt').val(), fundLimitPercentage:$('#fundLimitPercentage').val(), ordinalNum:$('#ordinalNum').val(), remarks:$('#remarks').val()},
            ...
            function updateSweepAccountAjax() {
				$.ajax({
				    type: 'POST',
				    data: {id:$('#sweepAccountId').val(), deposit:$('#depositAccount').val(), type:$('#type').val(), rule:$('#rule').val(), fundLimitAmt:$('#fundLimitAmt').val(), fundLimitPercentage:$('#fundLimitPercentage').val(), ordinalNum:$('#ordinalNum').val(), remarks:$('#remarks').val()},
            ...
            function showUpdateSweepAccount(id) { 
            	$.ajax({
				    type: 'POST',
				    data: {id:id},
            ...
            function deleteSweepAccountAjax(id) {
            	if (confirm('Are you sure?')) {
					$.ajax({
					    type: 'POST',
					    data: {id:id},
        	...
        	icbs.Dependencies.Ajax.addLoadEvent(function(){        		
        		$('#installment-tab').hide();  
        		updateLoanApplicationAjax({loanApplication2:"${loanInstance?.loanApplication?.id}"});
            });
   		</g:javascript>
	</head>
	<body>
		...
		<div id="edit-loan" class="content scaffold-edit" role="main">
			...
			<g:form id="loan-form" url="[resource:loanInstance, action:'update']" method="PUT" >
				<g:hiddenField name="version" value="${loanInstance?.version}" />
				<g:hiddenField name="activity" value="${(loanInstance?.performanceClassification?.id == 1 || loanInstance?.performanceClassification?.id == 4) ? 'Reschedule' : 'Restructure'}" />
				<div class="nav-tab-container">
                    <ul class="nav nav-tabs">
                        ...
                        <li><a href="#tab_7" data-toggle="tab">Sweep</a></li>
                    </ul>
                </div>
                <div class="tab-content">
                	<g:set var="reschedule" value="${true}" />
        ...
        <content tag="main-actions">
            <ul>
                <g:if test="${module == "loanRestructure" && (loanInstance?.loanPerformanceId.id == 2 || loanInstance?.loanPerformanceId.id == 3) }">
                <li><g:submitButton id="save" name="save" value="Save Restructured"/>
                <script type="text/javascript">
		            $(document).ready(function() {
		               	$( "#save" ).click(function() {
		             		 checkIfAllowed('LON01400', 'form#loan-form', 'Update branch XXX.', null); // params: policyTemplate.code, form to be saved
		               	});
		            }); 
                </script>
                </li>
                </g:if>   
                <g:elseif test="${module == "loanReschedule" && loanInstance?.loanPerformanceId.id == 1 && loanInstance?.status?.id == 4}">
               
                 <li><g:submitButton id="save" name="save" value="Save Reschedule" onclick="return confirm('${message(code: 'default.button.delete.confirm.message', default: 'Save?')}');"/>
                <script type="text/javascript">
		            $(document).ready(function() {
		               	$( "#save" ).click(function() {
		             		 checkIfAllowed('LON01300', 'form#loan-form', 'Update branch XXX.', null); // params: policyTemplate.code, form to be saved
		               	});
		            }); 
                </script>
                </li>
                </g:elseif>
                <li><g:link controller="${module}" action="show" id="${loanInstance.id}" onclick="return confirm('${message(code: 'default.button.delete.confirm.message', default: 'Are you sure?')}');">Cancel</g:link></li>
            </ul>
        </content>
	</body>
</html>
