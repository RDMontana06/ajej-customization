...
<div>
    ...
    <div name="add-deduction-form">
        <div class="fieldcontain form-group ${hasErrors(bean: deduction, field: 'scheme', 'has-error')}">
            ...
            <div class="col-sm-8">
                <g:select class="form-control" id="deductionScheme" name="scheme.id" from="${product?.loanDeductionSchemes?.findAll{it.status.id == 2}}" optionKey="id" optionValue="name" value="${deduction?.scheme?.id}" onchange="updateDeductionForm()" />
		...
		<div id="deduction-amount-form-group" class="fieldcontain form-group ${hasErrors(bean: deduction, field: 'amount', 'has-error')}" style="display:none">
			...
			<div class="col-sm-8">
                   <g:field type="number" name="deductionAmount" value="${deduction?.amount}" class="form-control truncated"/>                
		...
		<div class="fieldcontain form-group ${hasErrors(bean: deduction, field: 'rate', 'has-error')}">
			...
			<div class="col-sm-8">
				<g:field class="form-control" name="deductionRate" value="${deduction?.rate}" onkeyup="updateDeductionAmount(2)"/>
