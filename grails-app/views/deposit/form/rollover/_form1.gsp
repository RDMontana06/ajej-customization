...
<div id="rolloverForm">
  <!--  <g:textField id="roll1" name="roll1" value="${currentRollover?.type?.id}"/>
    ...
    <div class="fieldcontain form-group ${hasErrors(bean: depositInstance, field: 'currentRollover.endDate', 'has-error')} required">
            ...
            <div class="col-sm-8"><g:customDatePicker name="currentRollover.endDate" onchange="changeRolloverForm(document.getElementById('currentRollover.type').value)" precision="day" class="form-control"  value="${depositInstance ? depositInstance?.currentRollover?.endDate?:Branch?.get(1).runDate +depositInstance?.fixedDepositTermScheme?.value.toInteger():currentRollover?.endDate}"  />
