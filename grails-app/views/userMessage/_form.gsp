...
<div class="fieldcontain form-group ${hasErrors(bean: userMessageInstance, field: 'recipient', 'has-error')} required">
	...
	<div class="col-sm-8"><g:select id="recipient" name="recipient.id" from="${icbs.admin.UserMaster.list()}" optionKey="id" optionValue="name" required="true" value="${userMessageInstance?.recipient?.id}" class="many-to-one form-control"  noSelection="['': '']"/>
...
<div class="fieldcontain form-group ${hasErrors(bean: userMessageInstance, field: 'body', 'has-error')} ">
	...
	<div class="col-sm-8"><g:textArea name="body" id="body" value="${userMessageInstance?.body}" class="form-control textarea" rows="7"/>
            <script>
                 $(function(){
