...
<html>
    ...
    <body>
                            ...
                            <g:each in="${CashInBankLedger.findAllByCashInBank(cashInBankInstance)}" status="i" var="cashInBankLedgerInstance">
                                <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                                    ...
                                    <g:if test="${cashInBankLedgerInstance?.checkbook}">
                                        <td>
                                            <g:link action="chkDetails" controller="cashInBank" id="${cashInBankLedgerInstance?.checkbook?.id}" >${cashInBankLedgerInstance?.checkbook.checkNo}</g:link>
                                        </td>
                                    </g:if>
                                    <g:else>
                                        <td>N/A</td>
        ...
        <content tag="main-actions">
            <ul>
                <li><g:link action="show" controller="cashInBank" id="${cashInBankInstance.id}" >View Account Details</g:link></li>
