...
<html>
  <head>
    ...
    <asset:javascript src="plugins/datatables/dataTables.checkboxes.min.js"/>
                ...
                <li class="nav_item">
                  ...
                  <ul>
                    <g:each in="${configMenu}">
                      ...
                      <!-- <li><a href="${createLink(uri: it.uri)}">${it.uri}/${it.name}</a></li> -->
    ...
    <header class="header">
      <div id="user-bar" class="hidden-print">
        <ul>
          <g:if test="${session.user!=null}">
            ...
            <li><a href="${createLink(uri: '/userMessage')}"><span id="new" class="fa fa-envelope icon"></span> <span id = "label" >Messages</span> <span style="color: green;" class="secondary-label" id="unmsg">${unreadMessages}</span></a></li>
            <li><a href="${createLink(uri: '/policyException')}"><span id="al" class="fa fa-exclamation-circle icon"></span> <span  id = "al1">Alerts</span> <span style="color: green;" class="secondary-label" id="ppex">${pendingPolicyExceptions}</span></a></li>					 
      ...
      <g:if test="${session.user!=null}">
        <div id="breadcrumbs" class="hidden-print">
          ...
          <a href="#" id="cashoh" class="btn btn-success" onClick="GenerateTotals();" style="float:right; color: #001f3f;margin-right:13px;">Check on Hand: 0.00</a>
  ...
  <script type="text/javascript">
          ...
          success: function(data) {
            ...
            if(isAllowed == 'true') {
              ...
              if (description === undefined){
                ...
                var locCharOf = description.indexOf("@@");
                console.log("locCharOf: "+locCharOf);
                if(locCharOf > 0){
                  var xxDescriptiiiption = description.split('@@');
                  if(xxDescriptiiiption[0]=="" || xxDescriptiiiption[0]==null){

                  }else{

                  }
                  if(xxDescriptiiiption[1]=="" || xxDescriptiiiption[1]==null){

                  }else{
                    description = xxDescriptiiiption[1];
                  }
                }
              }
              
              $('#descriptionOverride').val(description);
    ...
    $(document).ready(function() {
      $( "#override_button" ).click(function() {
        $.ajax({
          method: "POST",
          data:{username:$('#usernameOverrider').val(), password:$('#passwordOverrider').val(), policyCode:$('#policyCode').val(), logDescription:$('#descriptionOverride').val(), overrideAmount:$('#overrideAmount').val()},
