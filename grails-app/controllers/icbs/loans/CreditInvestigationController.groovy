...
class CreditInvestigationController {
    ...
    //Print Loan Credit Summary AMLA Checklist
    def printAMLACheckList(CreditInvestigation creditInvestigationInstance){    
        try {    
            println"---------------- "
            params._name = "AMLA_CHECKLIST"
            params._format ="PDF"//required caps
            params._file ="amlactf/amlactf.jrxml" //jasper file name
            def getId = CreditInvestigation.get(creditInvestigationInstance.id).id
            params.id =  getId.toString()
