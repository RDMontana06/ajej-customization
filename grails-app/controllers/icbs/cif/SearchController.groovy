           ...
           return
            ...
            def list = Customer.createCriteria().list (params) {
                or{
                    ...
                    ilike("sssNo", "%${params.searchQuery}%")
                }
...
//      For Fixed Deposit
        if(type == 3){
            def fd = "SELECT A.id, C.passbook_line, C.txn_date, A.acct_no, " 
                fd += "A.date_opened, B.end_date, B.end_date - A.date_opened as Term, "
                fd += "A.interest_rate, C.debit_amt, C.credit_amt, C.bal "
                fd += "FROM deposit as A "
                fd += "LEFT JOIN rollover as B ON A.current_rollover_id = B.id "
                fd += "LEFT JOIN txn_deposit_acct_ledger as C ON A.id = C.acct_id "
                fd += "WHERE A.id = "+id
                fd += "and C.txn_date >= " +"'"+newstartdate +"' and C.txn_date <= '"+newenddate+"' and C.passbook_line is not null"
                def fdresult = sql.rows(fd)
            render(template:"searchTemplate/deposit/reprintGrid/viewGrid", model:[params:params,  domainInstanceList:fdresult])
        }       
        else{
            def results = TxnDepositAcctLedger.createCriteria().list{ 
                and {
                    params.sort = "id"
                    ge("txnDate", startdate)
            ...
            */
            render(template:"searchTemplate/deposit/reprintGrid/viewGrid", model:[params:params,  domainInstanceList: results]) as JSON
        }
    }
