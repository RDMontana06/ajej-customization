            ...
            if(acct1){ // Account exists!
                ...
                if((acct1.status == DepositStatus.get(5)) || (acct1.status == DepositStatus.get(7)))
                    ...
                    if((new String(txnType) == "30") || (new String(txnType) == "92")){ // INQUIRY || MINI STATEMENT
                        ...
                        if(!elementArray[2].equals("")){ // Txn amt
                            ...
                            if(params.reqCode == "0200"){
                                ...
                                if (!productTxnInstance){
                                ...
                                else if (atmTxnInstance.txnAmt > acct1.availableBalAmt){
                                    ...
                                    def txnFileInstance = new TxnFile(acctNo:acct1.acctNo, depAcct:acct1, currency:acct1.product.currency, 
                                        ...
                                        txnTimestamp:new Date().toTimestamp(), txnDescription:txnTemp.codeDescription, status:2, 
                                        txnParticulars:strTxnName, txnType:ATMTxnTemplate.txnType, txnTemplate:ATMTxnTemplate, user:ATMUSerID,
                                        txnRef: atmTxnReference)
                                    ...
                                    def acctLedgerInstance = new TxnDepositAcctLedger(acct:txnFileInstance.depAcctId, acctNo:acct1.acctNo, 
                                        ...
                                        bal:acct1.ledgerBalAmt,txnFile:txnFileInstance.id, currency:nCurrency, status:2, 
                                        txnType:ATMTxnTemplate.txnType, txnRef:strTxnName)
                                ...
                                if((acct2.status == DepositStatus.get(5)) || (acct2.status == DepositStatus.get(7)))
                                    ...
                                    if((new String(txnType) == "40") || (new String(txnType) == "21")){ // Fund Transfer
                                        if(params.reqCode == "0200"){
                                            ...
                                            def txnFile1Instance = new TxnFile(txnParticulars:AtmTable.findByAtmTxnCode(elementArray[1]).atmDescription,currency:acct1.currency,acctStatus:acct1.status.id,status:ConfigItemStatus.read(2),txnTimestamp:new Date().toTimestamp(),branch:branchInstance,acctNo:acct1.acctNo,txnAmt:atmTxnInstance.txnAmt,txnRef:atmTxnInstance.txnRef,depAcct:acct1)
                                            def txnFile2Instance = new TxnFile(txnParticulars:AtmTable.findByAtmTxnCode(elementArray[1]).atmDescription,currency:acct2.currency,acctStatus:acct2.status.id,status:ConfigItemStatus.read(2),txnTimestamp:new Date().toTimestamp(),branch:branch2Instance,acctNo:acct2.acctNo,txnAmt:atmTxnInstance.txnAmt,txnRef:atmTxnInstance.txnRef,depAcct:acct2)
                                            def acctLedger1Instance = new TxnDepositAcctLedger(branch:branchInstance,currency:acct1.currency,status:ConfigItemStatus.read(2),txnDate:new Date(),txnFile:txnFile1Instance,acct:acct1,acctNo:acct1.acctNo,debitAmt:atmTxnInstance.txnAmt,bal:acct1.availableBalAmt,txnRef:atmTxnInstance.txnRef)
                                            def acctLedger2Instance = new TxnDepositAcctLedger(branch:branch2Instance,currency:acct2.currency,status:ConfigItemStatus.read(2),txnDate:new Date(),txnFile:txnFile2Instance,acct:acct2,acctNo:acct2.acctNo,creditAmt:atmTxnInstance.txnAmt,bal:acct2.availableBalAmt,txnRef:atmTxnInstance.txnRef)
                                            txnFile1Instance.save(flush:true)
                                            txnFile2Instance.save(flush:true)
                                            acctLedger1Instance.save(flush:true)
                                            acctLedger2Instance.save(flush:true)
